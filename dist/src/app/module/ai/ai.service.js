...
const matchMentors = async (userId, goal, budget) => {
    const mentors = await prisma.mentor.findMany({
        where: { isDeleted: false, isActive: true },
        include: { user: { select: { name: true } } },
        take: 20,
    });
    const mentorSummaries = mentors.map(m => `ID:${m.id} Name:${m.user.name} Title:${m.title} Skills:${m.skills.join(",")} Rate:$${m.hourlyRate}/hr Rating:${m.averageRating}`).join("\n");
    ...
    const enriched = matches.map((m) => ({ ...m, mentor: mentors.find(mn => mn.id === m.mentorId) }));
    return enriched;
};
// 2. AI Chat Assistant
...
const generateSessionSummary = async (bookingId) => {
