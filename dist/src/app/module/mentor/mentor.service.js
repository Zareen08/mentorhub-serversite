...
const getAllMentors = async (query) => {
    ...
    if (query.minRate)
        where.hourlyRate = { ...(where.hourlyRate || {}), gte: Number(query.minRate) };
    if (query.maxRate)
        where.hourlyRate = { ...(where.hourlyRate || {}), lte: Number(query.maxRate) };
    if (query.minRating)
        where.averageRating = { gte: Number(query.minRating) };
...
const createMentorProfile = async (userId, payload) => {
    ...
    if (existing)
        throw new AppError(status.CONFLICT, "Mentor profile already exists");
