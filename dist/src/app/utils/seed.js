...
const seedUser = async (email, password, name, role) => {
    ...
    await prisma.user.update({ where: { id: result.user.id }, data: { emailVerified: true, role: role } });
    ...
    try {
        ...
        for (const m of extraMentors) {
            ...
            if (u) {
                ...
                if (!profileExists)
                    await prisma.userProfile.create({ data: { userId: u.id, expertise: m.skills.slice(0, 3) } });
        ...
        if (!demoUser) {
            ...
            if (demoUser) {
                ...
                if (!profileExists)
                    await prisma.userProfile.create({ data: { userId: demoUser.id, bio: "Looking to grow my career", expertise: ["JavaScript", "Python"] } });
